...
essays_df['unigrams'] = essays_df.apply(lambda x: re.sub(r'[^\w\s]|\r\n',' ',x.essay).lower().split(), axis=1)
stop = time.time()
print("({} s)\n".format(stop-start))
...
essays_df['tokens'] = essays_df.apply(lambda x: [token for token in trigram_mod[bigram_mod[x['unigrams']]] if token not in stopwords], axis=1)
...
essays_df['tokens'] = essays_df.apply(lambda x: [stemmer.stem(token) for token in x['tokens']], axis=1)
essays_df.to_csv("temp.csv")
stop = time.time()
print("({} s)\n".format(stop-start))
